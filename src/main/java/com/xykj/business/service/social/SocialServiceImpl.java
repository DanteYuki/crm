...
public class SocialServiceImpl extends BaseServiceImpl<Social> implements SocialService{
	...
	public void insert(Social o) {
		...
		//系统当前用户
		if(account != null) {
			o.setCreateBy(account.getRealName());
		} else {
			o.setCreateBy("");
		}
		...
		socialLog.setOperator(account.getRealName());
...
//		String filePath = "/root/crm_version1.0/tomcat8/webapps/ruisiyuan/demo/demo.xlsx";
			...
			for (int i = 1 ; i <= sheet.getLastRowNum(); i++) {
				...
				if(row != null) {
					...
					if (isDumplicated(social)) {
						Social social1 = socialDao.findByPhone(social);
						social.setUpdateBy(account.getRealName());
						social.setUpdateTime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
						social.setId(social1.getId());
						socialDao.update(social);
					} else {
						social.setAdvisor(account.getRealName());
						this.insert(social);
					}
				}
			}
	...
	public Page<Social> findTry(Page<Social> page, Social social) {
		...
		Account account = tokenManager.getCurrentAccount(social.getToken());
		social.setCampusId(account.getCampusId());
		return super.findByPage(page, social);
	}
	...
	public void goIntoClass(String idString, String classId) {
		if (idString != null && !idString.trim().equals("")) {
			...
			for (String id : ids) {
				...
				social.setId(id);
				//查询出当个学生的社招信息
				social = socialDao.find(social);
				ClassStudent classStudent = new ClassStudent();
				classStudent.setId(UuidUtil.get32UUID());
				classStudent.setName(social.getSname());
				classStudent.setAge(social.getSage());
				classStudent.setGender(social.getSgender().trim().equals("男") ? 1 : 0);
				classStudent.setMobile(social.getMobile());
				classStudent.setQq(social.getQq());
				classStudent.setWx(social.getWx());
				classStudent.setEducation(social.getEducation());
				classStudent.setRemark("");
				classStudent.setClassId(classId);
				classStudent.setCredit(100);
				classStudent.setSocialId(social.getId());
				classStudent.setChannelId("");
				classStudent.setReputationId("");
				classStudent.setAdvisorFeedback(social.getFeedback());
				social.setStatus(6);
				socialDao.disabled(social);
	...
	public void addAdvisorFeedback(Social social) {
		socialDao.insertFeedback(social);
	}
	...
	public void divide(String socialIds, String advisor) {
		...
		for (String id : ids) {
			if (id != null && !id.trim().equals("")) {
				Social o = new Social();
				o.setId(id);
				Social social = socialDao.find(o);
				social.setAdvisor(advisor);
				if (o != null) {
					socialDao.divide(social);
				}
			}
		}
	}
